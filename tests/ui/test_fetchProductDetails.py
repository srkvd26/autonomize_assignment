...
def navHPage(driver, logger):
    logger.info("===== STARTED GET PRODUCT DETAILS TEST =====")
    ...
    try:
        ...
        logger.info("No 'Continue shopping' prompt displayed")
...
def search(driver, logger):
    ...
    logger.info(f"Entered search keyword: {search_keyword}")
...
def saveIntoFile(logger):
    ExcelReadWrite.write(output_file, prdList)
    logger.info(f"Saved product details into file: {output_file}")
...
def productCnt(logger):
    ...
    logger.info("===== GET PRODUCT DETAILS TEST ENDED =====")
